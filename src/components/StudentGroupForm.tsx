...
export function StudentGroupForm({ onSubmit, onCancel, initialData, isLoading }: StudentGroupFormProps) {
  const [formData, setFormData] = useState<StudentGroupFormData>({
    groupName: initialData?.groupName || '',
    startHour: initialData?.startHour || 8,
    startMinute: initialData?.startMinute || 10,
    endHour: initialData?.endHour || 15,
    endMinute: initialData?.endMinute || 30
  })
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <input
          ...
          className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.groupName ? 'border-red-500' : 'border-gray-300'
            }`}
      ...
      <div>
        <TimingFields
          ...
          description="Set the working day hours for this student group. This determines when group members can be scheduled."
        />
        {errors.timing && (
