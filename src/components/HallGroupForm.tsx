...
import { HallGroupFormData } from '@/types'

interface HallGroupFormProps {
  onSubmit: (data: HallGroupFormData) => void
  onCancel: () => void
  initialData?: Partial<HallGroupFormData>
  isLoading?: boolean
}

export function HallGroupForm({ onSubmit, onCancel, initialData, isLoading }: HallGroupFormProps) {
  const [formData, setFormData] = useState<HallGroupFormData>({
    groupName: initialData?.groupName || '',
    startHour: initialData?.startHour || 8,
    startMinute: initialData?.startMinute || 10,
    endHour: initialData?.endHour || 20,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <input
          ...
          placeholder="Enter group name (e.g., Main Building, Lab Halls, Lecture Halls)"
          disabled={isLoading}
        />
        {errors.groupName && (
      ...
      <div>
        <TimingFields
          ...
          title="Group Availability Hours"
          description="Set the availability hours for this hall group. This determines when group halls can be scheduled."
        />
        {errors.timing && (
