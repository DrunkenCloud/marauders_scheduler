...
interface StudentGroupListProps {
  ...
  onManageMembers: (group: StudentGroup) => void
  onViewTimetable: (group: StudentGroup) => void
  isLoading?: boolean
}

export function StudentGroupList({ groups, onEdit, onDelete, onManageMembers, onViewTimetable, isLoading }: StudentGroupListProps) {
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        case 'groupName':
          ...
          break
        case 'memberCount':
          aValue = a._count.studentMemberships
          bValue = b._count.studentMemberships
          break
        case 'createdAt':
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-600">
        Showing {filteredAndSortedGroups.length} of {groups.length} student groups
      </div>

      {/* Groups List */}
      {filteredAndSortedGroups.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          {searchTerm ? 'No student groups match your search.' : 'No student groups found.'}
        </div>
      ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {group._count.studentMemberships} members
                    </span>
                  </div>
